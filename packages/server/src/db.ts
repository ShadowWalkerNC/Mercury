...
migrate(`CREATE TABLE IF NOT EXISTS schema_meta (key TEXT PRIMARY KEY, value TEXT NOT NULL);`);
...
// M-032: additive columns for existing deployments
migrate(`ALTER TABLE users ADD COLUMN is_admin     INTEGER NOT NULL DEFAULT 0;`);
...
migrate(`CREATE TABLE IF NOT EXISTS sessions (
  ...
  refresh_token TEXT UNIQUE NOT NULL, expires_at TEXT NOT NULL,
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
);`);
...
migrate(`CREATE TABLE IF NOT EXISTS members (
  ...
  joined_at TEXT NOT NULL DEFAULT (datetime('now')), UNIQUE (space_id, user_id)
);`);
...
migrate(`CREATE TABLE IF NOT EXISTS messages (
  ...
  edited_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now'))
);`);
...
migrate(`CREATE TABLE IF NOT EXISTS attachments (
  ...
  url TEXT NOT NULL, filename TEXT NOT NULL, size INTEGER NOT NULL,
  mime_type TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT (datetime('now'))
);`);
...
migrate(`CREATE TABLE IF NOT EXISTS invites (
  ...
  creator_id TEXT NOT NULL REFERENCES users(id), uses INTEGER NOT NULL DEFAULT 0,
  max_uses INTEGER, expires_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now'))
);`);

migrate(`CREATE VIRTUAL TABLE IF NOT EXISTS messages_fts USING fts5(
  content, content=messages, content_rowid=rowid,
  tokenize='unicode61 remove_diacritics 2'
);`);
migrate(`CREATE TRIGGER IF NOT EXISTS messages_ai AFTER INSERT ON messages BEGIN
  INSERT INTO messages_fts(rowid, content) VALUES (new.rowid, new.content); END;`);
migrate(`CREATE TRIGGER IF NOT EXISTS messages_ad AFTER DELETE ON messages BEGIN
  INSERT INTO messages_fts(messages_fts, rowid, content) VALUES ('delete', old.rowid, old.content); END;`);
migrate(`CREATE TRIGGER IF NOT EXISTS messages_au AFTER UPDATE ON messages BEGIN
